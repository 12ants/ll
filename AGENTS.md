...
- `npm run dev` — Vite dev server on `localhost:5173`, binds `0.0.0.0`.
- `npm test` — Vitest (`tests/**/*.test.ts`). Fast unit tests for config, geography, style.
- `npm run build` — **Runs `tsc -b` first, then `vite build`.** A type error fails the build before bundling.
- `npm run preview` — serve production bundle.
- `npm run test:browser` — Playwright against the dev server. Install Chromium once with `npx playwright install chromium`. Needs `npm run dev` in another terminal. Slow; uses real public tiles and software WebGL. Writes captures to `.artifacts/`.
